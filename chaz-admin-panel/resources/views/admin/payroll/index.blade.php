...
<div class="page-header">
    ...
    <a href="{{ route('admin.payroll.create-period') }}" class="btn btn-primary"><i class="fas fa-plus"></i> New Period</a>
    @endif
</div>
        ...
        <table>
            ...
            <tr>
                <td>
                    ...
                    <div style="font-size:.75rem;color:var(--text-muted);">{{ $period->start_date?->format('d M') }} — {{ $period->end_date?->format('d M Y') }}</div>
                </td>
                ...
                <td>
                    @if(admin_can('manage_payroll'))
                    @if($period->isOpen())
                    <form method="POST" action="{{ route('admin.payroll.run', $period) }}" style="display:inline;" onsubmit="return confirm('Run payroll for {{ $period->name }}? This will generate payslips for all active employees.')">
                        @csrf
