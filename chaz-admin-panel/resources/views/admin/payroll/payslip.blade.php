...
<div class="page-header">
    ...
    <div style="display:flex;gap:8px;">
        ...
        <a href="{{ route('admin.payroll.run', $payslip->payrollRun) }}" class="btn btn-outline"><i class="fas fa-arrow-left"></i> Back</a>
    </div>
</div>
    ...
    <div class="card-body">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:20px;">
            ...
            <div>
                ...
                <div style="font-size:.78rem;color:var(--text-muted);margin-top:8px;">Payment Date</div>
                <div>{{ $payslip->payrollRun->payrollPeriod->payment_date?->format('d M Y') ?? '—' }}</div>
        ...
        <table style="width:100%;margin-bottom:16px;">
            ...
            <tr><td style="padding:7px;">{{ $item->name }}</td><td style="text-align:right;">{{ number_format($item->amount, 2) }}</td></tr>
            @endforeach
            <tr><td style="padding:7px;">NAPSA (Employee 5%)</td><td style="text-align:right;">{{ number_format($payslip->napsa_employee, 2) }}</td></tr>
            <tr><td style="padding:7px;">NHIMA (Employee 1%)</td><td style="text-align:right;">{{ number_format($payslip->nhima_employee, 2) }}</td></tr>
            <tr><td style="padding:7px;">PAYE (Income Tax)</td><td style="text-align:right;">{{ number_format($payslip->paye, 2) }}</td></tr>
            <tr style="font-weight:700;background:var(--bg);">
        ...
        <div style="background:var(--primary);color:#fff;border-radius:8px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center;">
            ...
            <span style="font-size:1.5rem;font-weight:800;">ZMW {{ number_format($payslip->net_salary, 2) }}</span>
        </div>

        {{-- Employer contributions --}}
        <div style="margin-top:14px;padding:12px;background:var(--bg);border-radius:6px;font-size:.78rem;color:var(--text-muted);">
            Employer Contributions (not deducted from pay): NAPSA {{ number_format($payslip->napsa_employer, 2) }} | NHIMA {{ number_format($payslip->nhima_employer, 2) }}
        </div>
