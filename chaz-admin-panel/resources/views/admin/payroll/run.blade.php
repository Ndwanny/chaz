...
<div class="page-header">
    <div>
        ...
        <div class="page-subtitle">Run by {{ $run->runBy->name ?? '—' }} on {{ $run->run_at?->format('d M Y H:i') }}</div>
    </div>
    <div style="display:flex;gap:8px;">
        @if($run->isDraft() && admin_can('manage_payroll'))
        <form method="POST" action="{{ route('admin.payroll.approve', $run) }}" onsubmit="return confirm('Approve this payroll run?')">
            @csrf @method('PATCH')
...
<div class="stats-grid" style="margin-bottom:24px;">
    ...
    <div class="stat-card"><div class="stat-icon green"><i class="fas fa-arrow-up"></i></div><div><div class="stat-value">{{ number_format($run->total_gross/1000,0) }}K</div><div class="stat-label">Total Gross (ZMW)</div></div></div>
    ...
    <div class="table-wrap">
        <table>
            ...
            <tr>
                ...
                <td style="font-weight:700;">{{ number_format($slip->net_salary, 2) }}</td>
