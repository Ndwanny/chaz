@extends('layouts.app')
@section('title', 'Employee Portal — CHAZ')
@section('meta_description', 'The CHAZ Employee Self-Service Portal — your one-stop hub for payslips, leave management, HR policies, training resources, and more.')
...
<style>
    ...
    .portal-login-card p { font-size: 0.875rem; opacity: 0.7; margin-bottom: 1.75rem; }
    .portal-input {
        width: 100%;
    ...
    .metric-item__label { font-size: 0.82rem; color: rgba(255,255,255,0.65); font-weight: 400; }
...
<section class="home-hero portal-hero">
    ...
    <div class="container">
        <div style="display:grid; grid-template-columns:1.1fr 0.9fr; gap:4rem; align-items:center;" class="portal-hero-grid">
            ...
            <div class="home-hero__content">
                ...
                <div class="home-hero__stats" style="margin-top:2.5rem;">
                    ...
                    <div class="stat-card fade-in">
                        <div class="stat-card__num">{{ $m['value'] }}</div>
            ...
            <div id="portal-login" class="portal-login-card fade-in">
                ...
                <form action="#" method="POST">
                    ...
                    <div style="position:relative; margin-bottom:0;">
                        <i class="fa fa-envelope" style="position:absolute;left:1rem;top:50%;transform:translateY(-50%);color:rgba(255,255,255,0.4);font-size:0.85rem;"></i>
                        <input type="email" name="email" class="portal-input" style="padding-left:2.5rem;" placeholder="staff.id@chaz.org.zm" required>
                    </div>
                    <div style="position:relative; margin-bottom:0;">
...
<section class="section section--cream" style="padding:3rem 0;">
    <div class="container">
        ...
        <div class="quick-links-grid">
            ...
            <a href="#" class="quick-link-btn quick-link-btn--{{ $link['color'] }} fade-in">
                <div class="quick-link-btn__icon icon--{{ $link['color'] }}">
                    <i class="fa {{ $link['icon'] }}"></i>
                </div>
                <span class="quick-link-btn__label">{{ $link['label'] }}</span>
            </a>
            @endforeach
        </div>
    </div>
...
<section class="section" id="about-portal">
    <div class="container" style="display:grid;grid-template-columns:1fr 1fr;gap:5rem;align-items:center;">
        ...
        <div class="fade-in" style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
            @foreach([
            ...
            <div style="background:var(--color-cream);border-radius:var(--radius-md);padding:1.5rem;text-align:center;transition:all var(--transition);" onmouseover="this.style.boxShadow='var(--shadow-md)';this.style.transform='translateY(-3px)'" onmouseout="this.style.boxShadow='';this.style.transform=''">
                ...
                <div style="font-weight:700;font-size:0.9rem;color:var(--color-forest);margin-bottom:0.2rem;">{{ $item['label'] }}</div>
                <div style="font-size:0.77rem;color:var(--color-slate-mid);">{{ $item['sub'] }}</div>
            </div>
            @endforeach
        </div>
    </div>
...
<section class="section section--cream" id="features">
    <div class="container">
        ...
        <div class="features-grid">
            ...
            <div class="feature-card feature-card--{{ $f['color'] }} fade-in">
                <div class="feature-card__icon icon--{{ $f['color'] }}">
                    <i class="fa {{ $f['icon'] }}"></i>
                </div>
...
<section class="section" id="benefits">
    <div class="container">
        <div style="display:grid;grid-template-columns:1fr 1.6fr;gap:5rem;align-items:center;">
            <div class="fade-in">
            ...
            <div class="benefits-grid fade-in">
                ...
                <div class="benefit-item">
                    <div class="benefit-item__icon"><i class="fa {{ $b['icon'] }}"></i></div>
                    <div>
    ...
    <div class="container">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1.5rem;max-width:960px;margin:0 auto;">
            ...
            <div style="background:white;border:1px solid var(--color-border);border-radius:var(--radius-md);padding:1.75rem;transition:all var(--transition);" class="fade-in" onmouseover="this.style.boxShadow='var(--shadow-md)';this.style.transform='translateY(-4px)'" onmouseout="this.style.boxShadow='';this.style.transform=''">
                ...
                <h4 style="font-family:var(--font-display);font-size:1rem;font-weight:700;color:var(--color-forest);margin-bottom:0.5rem;">{{ $metric['title'] }}</h4>
                <p style="font-size:0.83rem;color:var(--color-slate-mid);line-height:1.7;">{{ $metric['desc'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="container">
        <div style="display:grid;grid-template-columns:1fr 2fr;gap:5rem;align-items:center;">
            <div class="fade-in">
                ...
                <a href="{{ route('contact') }}" class="btn btn--gold" style="margin-top:2rem;">
            ...
            <div class="security-grid fade-in">
                ...
                <div class="security-item">
                    <div class="security-item__icon"><i class="fa {{ $sec['icon'] }}"></i></div>
                    <div>
                        <div class="security-item__title">{{ $sec['title'] }}</div>
                        <div class="security-item__desc">{{ $sec['desc'] }}</div>
                    </div>
                </div>
                @endforeach
            </div>
...
<section class="section" id="get-started">
    <div class="container">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:2rem;max-width:1000px;margin:0 auto 3rem;position:relative;">
            {{-- Connector line --}}
            <div style="position:absolute;top:28px;left:12.5%;right:12.5%;height:2px;background:linear-gradient(90deg,var(--color-forest),var(--color-gold));z-index:0;border-radius:2px;"></div>
            ...
            <div class="fade-in" style="text-align:center;position:relative;z-index:1;">
                <div style="width:56px;height:56px;background:var(--color-forest);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 1.25rem;border:4px solid var(--color-cream);box-shadow:0 0 0 3px var(--color-forest);">
                    <i class="fa {{ $step['icon'] }}" style="color:var(--color-gold);font-size:1.1rem;"></i>
                </div>
                <div style="font-size:0.7rem;font-weight:700;letter-spacing:0.1em;color:var(--color-gold);text-transform:uppercase;margin-bottom:0.4rem;">Step {{ $step['num'] }}</div>
                <h4 style="font-family:var(--font-display);font-size:1rem;font-weight:700;color:var(--color-forest);margin-bottom:0.5rem;">{{ $step['title'] }}</h4>
                <p style="font-size:0.82rem;color:var(--color-slate-mid);line-height:1.7;">{{ $step['desc'] }}</p>
            </div>
            @endforeach
        </div>
