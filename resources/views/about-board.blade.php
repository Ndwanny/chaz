@extends('layouts.app')
@section('title', 'Board of Trustees — CHAZ')
...
<div class="page-hero">
    <div class="container">
        ...
        <div class="page-hero__breadcrumb">
            <a href="{{ route('home') }}">Home</a> <i class="fa fa-chevron-right" style="font-size:0.65rem"></i>
            <a href="{{ route('about') }}">About</a> <i class="fa fa-chevron-right" style="font-size:0.65rem"></i>
            Board of Trustees
        </div>
    </div>
</div>
    ...
    <div class="container">
        ...
        <div class="board-grid">
            ...
            <div class="board-card fade-in">
                <div class="board-card__avatar">{{ $t['initials'] }}</div>
                <div class="board-card__name">{{ $t['name'] }}</div>
                <div class="board-card__role">{{ $t['role'] }}</div>
                <div class="board-card__org">{{ $t['org'] }}</div>
            </div>
            @endforeach
        </div>
        ...
        <div style="background:var(--color-cream); border-radius:var(--radius-md); padding:2.5rem; margin-top:3rem; text-align:center;" class="fade-in">
            ...
            <div style="display:flex; flex-wrap:wrap; gap:0.75rem; justify-content:center;">
                @foreach(['Medical & Clinical Affairs', 'Pharmacy & Supply Chain', 'Finance & Audit', 'Human Resources & Governance'] as $com)
                <span class="badge badge--green" style="font-size:0.8rem;padding:0.4rem 1rem;">{{ $com }}</span>
                @endforeach
            </div>
        </div>
    </div>
