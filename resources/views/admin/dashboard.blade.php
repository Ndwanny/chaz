...
<div class="stats-grid" style="margin-bottom:1.75rem;">
    <div class="stat-box">
        ...
        <div>
            <div class="stat-box__num">{{ number_format($stats['news']) }}</div>
            <div class="stat-box__label">News Articles</div>
            <div class="stat-box__sub">{{ $stats['news_published'] }} published</div>
        </div>
    </div>
    <div class="stat-box">
        <div class="stat-box__icon stat-box__icon--gold"><i class="fa fa-briefcase"></i></div>
        <div>
            <div class="stat-box__num">{{ number_format($stats['jobs']) }}</div>
            <div class="stat-box__label">Job Postings</div>
            <div class="stat-box__sub">{{ $stats['jobs_open'] }} open</div>
        </div>
    </div>
    <div class="stat-box">
        <div class="stat-box__icon stat-box__icon--blue"><i class="fa fa-file-contract"></i></div>
        <div>
            <div class="stat-box__num">{{ number_format($stats['tenders']) }}</div>
            <div class="stat-box__label">Tenders</div>
            <div class="stat-box__sub">{{ $stats['tenders_open'] }} open</div>
        </div>
    </div>
    <div class="stat-box">
        <div class="stat-box__icon stat-box__icon--red"><i class="fa fa-envelope"></i></div>
        <div>
            <div class="stat-box__num">{{ number_format($stats['messages']) }}</div>
            <div class="stat-box__label">Messages</div>
            <div class="stat-box__sub {{ $stats['unread_messages'] > 0 ? 'text-warning' : '' }}">{{ $stats['unread_messages'] }} unread</div>
        </div>
    </div>
    <div class="stat-box">
        <div class="stat-box__icon stat-box__icon--green"><i class="fa fa-hospital"></i></div>
        <div>
            <div class="stat-box__num">{{ number_format($stats['members']) }}</div>
    ...
    <div class="stat-box">
        ...
        <div>
            <div class="stat-box__num">{{ number_format($stats['downloads']) }}</div>
...
<div class="card" style="margin-bottom:1.75rem;">
    ...
    <div class="card-body" style="display:flex;flex-wrap:wrap;gap:0.75rem;">
        <a href="{{ route('admin.news.create') }}"    class="btn btn-forest"><i class="fa fa-plus"></i> New Article</a>
        <a href="{{ route('admin.jobs.create') }}"    class="btn btn-forest"><i class="fa fa-plus"></i> Post a Job</a>
        <a href="{{ route('admin.tenders.create') }}" class="btn btn-forest"><i class="fa fa-plus"></i> Add Tender</a>
        <a href="{{ route('admin.announcements.create') }}" class="btn btn-outline"><i class="fa fa-bullhorn"></i> Announcement</a>
        <a href="{{ route('admin.leave.index') }}" class="btn btn-outline"><i class="fa fa-calendar"></i>
            Leave Requests @if($stats['pending_leave'] > 0)<span class="badge badge-red" style="margin-left:4px;">{{ $stats['pending_leave'] }}</span>@endif
        </a>
        @if($stats['unread_messages'] > 0)
        <a href="{{ route('admin.messages.index') }}" class="btn btn-gold">
            <i class="fa fa-envelope"></i> {{ $stats['unread_messages'] }} Unread
        </a>
        @endif
    </div>
</div>
...
<div class="stats-grid" style="margin-bottom:1.25rem;">
    <div class="stat-box" onclick="location.href='{{ route('admin.employees.index') }}'" style="cursor:pointer;">
        <div class="stat-box__icon stat-box__icon--blue"><i class="fa fa-id-badge"></i></div>
        <div>
            <div class="stat-box__num" data-kpi="hr.active_employees">{{ number_format($stats['total_employees']) }}</div>
            <div class="stat-box__label">Active Employees</div>
            <div class="stat-box__sub" data-kpi-sub="hr.on_leave_today">{{ $stats['on_leave_today'] }} on leave today</div>
        </div>
    </div>
    <div class="stat-box {{ $stats['pending_leave'] > 0 ? 'stat-box--alert' : '' }}" onclick="location.href='{{ route('admin.leave.index') }}?status=pending'" style="cursor:pointer;">
        <div class="stat-box__icon stat-box__icon--gold"><i class="fa fa-calendar-minus"></i></div>
        <div>
            <div class="stat-box__num" data-kpi="hr.pending_leave">{{ number_format($stats['pending_leave']) }}</div>
            <div class="stat-box__label">Pending Leave</div>
            <div class="stat-box__sub">Awaiting approval</div>
        </div>
    </div>
    <div class="stat-box" onclick="location.href='{{ route('admin.payroll.index') }}'" style="cursor:pointer;">
        <div class="stat-box__icon stat-box__icon--green"><i class="fa fa-money-bill-wave"></i></div>
        <div>
            <div class="stat-box__num" data-kpi="payroll.last_run_net" data-kpi-format="zmw-k">ZMW {{ number_format($stats['last_payroll_net']/1000,0) }}K</div>
            <div class="stat-box__label">Last Payroll</div>
            <div class="stat-box__sub" data-kpi-sub="payroll.last_run_month">{{ $stats['last_payroll_month'] }}</div>
        </div>
    </div>
    <div class="stat-box" onclick="location.href='{{ route('admin.portal-accounts.index') }}'" style="cursor:pointer;">
        <div class="stat-box__icon stat-box__icon--teal"><i class="fa fa-door-open"></i></div>
        <div>
            <div class="stat-box__num" data-kpi="portal.portal_active">{{ number_format($stats['total_employees']) }}</div>
...
<div class="stats-grid" style="margin-bottom:1.25rem;">
    <div class="stat-box {{ $stats['pending_pos'] > 0 ? 'stat-box--alert' : '' }}" onclick="location.href='{{ route('admin.purchase-orders.index') }}'" style="cursor:pointer;">
        <div class="stat-box__icon stat-box__icon--gold"><i class="fa fa-file-invoice"></i></div>
        <div>
            <div class="stat-box__num" data-kpi="procurement.pending_pos">{{ number_format($stats['pending_pos']) }}</div>
            <div class="stat-box__label">Pending POs</div>
            <div class="stat-box__sub">Awaiting approval</div>
        </div>
    </div>
    <div class="stat-box" onclick="location.href='{{ route('admin.inventory.index') }}'" style="cursor:pointer;">
        <div class="stat-box__icon stat-box__icon--blue"><i class="fa fa-warehouse"></i></div>
        <div>
            <div class="stat-box__num" data-kpi="procurement.total_items">{{ number_format($stats['low_stock'] + $stats['out_of_stock']) }}</div>
            <div class="stat-box__label">Stock Alerts</div>
            <div class="stat-box__sub" data-kpi-sub="procurement.low_stock-low-stock">{{ $stats['low_stock'] }} low · {{ $stats['out_of_stock'] }} out</div>
        </div>
    </div>
    <div class="stat-box" onclick="location.href='{{ route('admin.suppliers.index') }}'" style="cursor:pointer;">
...
<div class="stats-grid" style="margin-bottom:1.75rem;">
    <div class="kpi-group">
        ...
        <div class="stats-grid stats-grid--tight">
            <div class="stat-box" onclick="location.href='{{ route('admin.fleet.vehicles.index') }}'" style="cursor:pointer;">
                <div class="stat-box__icon stat-box__icon--teal"><i class="fa fa-car"></i></div>
                <div>
                    <div class="stat-box__num" data-kpi="fleet.active_vehicles">{{ number_format($stats['active_vehicles']) }}</div>
                    <div class="stat-box__label">Active Vehicles</div>
                    <div class="stat-box__sub" data-kpi-sub="fleet.total_vehicles-total">{{ $stats['total_vehicles'] }} total fleet</div>
                </div>
            </div>
            <div class="stat-box {{ $stats['maintenance_due'] > 0 ? 'stat-box--alert' : '' }}">
                <div class="stat-box__icon stat-box__icon--red"><i class="fa fa-wrench"></i></div>
                <div>
                    <div class="stat-box__num" data-kpi="fleet.maintenance_due">{{ number_format($stats['maintenance_due']) }}</div>
    ...
    <div class="kpi-group">
        ...
        <div class="stats-grid stats-grid--tight">
            <div class="stat-box {{ $stats['pending_expenses'] > 0 ? 'stat-box--alert' : '' }}" onclick="location.href='{{ route('admin.finance.expenses.index') }}'" style="cursor:pointer;">
                <div class="stat-box__icon stat-box__icon--red"><i class="fa fa-receipt"></i></div>
                <div>
                    <div class="stat-box__num" data-kpi="finance.pending_count">{{ number_format($stats['pending_expenses']) }}</div>
                    <div class="stat-box__label">Pending Expenses</div>
                    <div class="stat-box__sub" data-kpi-sub="finance.pending_amount-zmw">ZMW {{ number_format($stats['pending_expenses_amount'], 0) }}</div>
                </div>
            </div>
            <div class="stat-box">
                <div class="stat-box__icon stat-box__icon--blue"><i class="fa fa-chart-line"></i></div>
                <div>
                    <div class="stat-box__num" data-kpi="finance.ytd_expenses" data-kpi-format="zmw-k">ZMW {{ number_format($stats['ytd_expenses']/1000,0) }}K</div>
                    <div class="stat-box__label">YTD Expenses</div>
                    <div class="stat-box__sub">{{ now()->year }}</div>
                </div>
            </div>
            <div class="stat-box" onclick="location.href='{{ route('admin.finance.budgets.index') }}'" style="cursor:pointer;">
...
<div class="form-grid form-grid--2" style="gap:1.5rem;margin-bottom:1.5rem;">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <a href="{{ route('admin.news.index') }}" class="btn btn-outline btn-sm">View All</a>
        </div>
            ...
            <table>
                ...
                <tr>
                    <td style="max-width:220px;">
                        <div style="font-weight:600;font-size:0.83rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $article->title }}</div>
                        <div style="font-size:0.75rem;color:var(--slate-mid);">{{ $article->tag }}</div>
                    </td>
                    <td><span class="badge {{ $article->status === 'published' ? 'badge-green' : 'badge-grey' }}">{{ $article->status }}</span></td>
                    <td style="font-size:0.78rem;color:var(--slate-mid);white-space:nowrap;">{{ $article->created_at->format('d M Y') }}</td>
                    <td><a href="{{ route('admin.news.edit', $article) }}" class="btn btn-outline btn-sm btn-icon"><i class="fa fa-pen"></i></a></td>
                </tr>
                @endforeach
                </tbody>
            </table>
    ...
    <div class="card">
        <div class="card-header">
            <h3><i class="fa fa-calendar-minus" style="color:var(--forest);margin-right:0.4rem;"></i>
                Pending Leave
                @if($stats['pending_leave'] > 0)<span class="badge badge-red" style="margin-left:6px;">{{ $stats['pending_leave'] }}</span>@endif
            </h3>
            <a href="{{ route('admin.leave.index') }}" class="btn btn-outline btn-sm">View All</a>
        </div>
        <div class="table-wrap">
            @if($recentLeave->count())
            <table>
                <thead><tr><th>Employee</th><th>Type</th><th>Dates</th><th></th></tr></thead>
                <tbody>
                @foreach($recentLeave as $lr)
                <tr>
                    <td>
                        <div style="font-weight:600;font-size:.83rem;">{{ $lr->employee->full_name ?? '—' }}</div>
                        <div style="font-size:.72rem;color:var(--slate-mid);">{{ $lr->employee->department->name ?? '' }}</div>
                    </td>
                    <td style="font-size:.82rem;">{{ $lr->leaveType->name ?? '—' }}</td>
                    <td style="font-size:.78rem;color:var(--slate-mid);">
                        {{ $lr->start_date?->format('d M') }} – {{ $lr->end_date?->format('d M') }}
                        <div style="font-size:.7rem;">({{ $lr->days_requested }} days)</div>
                    </td>
                    <td>
                        <a href="{{ route('admin.leave.index') }}" class="btn btn-outline btn-xs"><i class="fa fa-eye"></i></a>
                    </td>
                </tr>
                @endforeach
                </tbody>
...
<div class="form-grid form-grid--2" style="gap:1.5rem;margin-bottom:1.5rem;">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <a href="{{ route('admin.finance.expenses.index') }}" class="btn btn-outline btn-sm">View All</a>
        </div>
        <div class="table-wrap">
            ...
            <table>
                ...
                @php $sc = ['submitted'=>'badge-gold','approved'=>'badge-blue','paid'=>'badge-success','rejected'=>'badge-red','draft'=>'badge-grey'][$exp->status] ?? 'badge-grey'; @endphp
                <tr>
                    <td style="font-size:.83rem;max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $exp->description }}</td>
                    <td style="font-size:.75rem;color:var(--slate-mid);">{{ $exp->department->name ?? '—' }}</td>
                    <td style="font-weight:600;font-size:.83rem;white-space:nowrap;">ZMW {{ number_format($exp->amount, 0) }}</td>
                    <td><span class="badge {{ $sc }}">{{ ucfirst($exp->status) }}</span></td>
                </tr>
                @endforeach
                </tbody>
            </table>
    ...
    <div class="card">
        ...
        <div class="table-wrap">
            ...
            <table>
                ...
                <tr style="{{ !$msg->read ? 'background:#FFFDF5;' : '' }}">
                    <td>
                        <div style="font-weight:{{ $msg->read ? '400' : '700' }};font-size:.83rem;">{{ $msg->name }}</div>
                        <div style="font-size:.75rem;color:var(--slate-mid);">{{ $msg->email }}</div>
                    </td>
                    ...
                    <td style="font-size:.78rem;color:var(--slate-mid);white-space:nowrap;">{{ $msg->created_at->format('d M') }}</td>
                    <td><a href="{{ route('admin.messages.show', $msg) }}" class="btn btn-outline btn-sm btn-icon"><i class="fa fa-eye"></i></a></td>
                </tr>
                @endforeach
                </tbody>
            </table>
...
@push('scripts')
<script>
const KPI_URL = '{{ route('admin.api.kpis') }}';
let kpiTimer;
